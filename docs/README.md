# Documentation

## Use Diptych Picker

- [Getting started](GETTING_STARTED.md)
- [User guide](USER_GUIDE.md)
- [Agent mode](AGENT_MODE.md)
- [Configuration](CONFIGURATION.md)
- [Data and recovery](DATA_AND_RECOVERY.md)
- [Local API](API.md)
- [Examples](../examples/README.md)

## Understand the v1.0 contract

- [Feature matrix](FEATURE_MATRIX.md)
- [Security model](../SECURITY.md)
- [Seed image provenance and licensing](../public/seed-assets/README.md)
- [Release checklist](RELEASE_CHECKLIST.md)
- [Diptych Picker 1.0.0 release record](releases/1.0.0.md)
- [Changelog](../CHANGELOG.md)

## Contribute and maintain

- [Contributing](../CONTRIBUTING.md)
- [Dependency policy](DEPENDENCY_POLICY.md)
- [Codebase review](CODEBASE_REVIEW_2026-07-21.md)
- [Product notes](PRODUCT_NOTES.md)

## Proposals and implementation records

- [Co-proc agent transport and prompt deck](CO_PROC_DECK_DESIGN.md)
- [Design specifications](superpowers/specs/)
- [Implementation plans](superpowers/plans/)

Proposal and plan documents are historical or future-facing unless the feature
matrix marks the behavior as shipped.
