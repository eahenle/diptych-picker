...
- Read `preferenceProfile.adaptationMode` when present; legacy jobs without a structured profile are Static. For Static jobs, omit `preferenceRevision` and do not mutate any preference field. For Adaptive jobs, write a complete `preferenceRevision` conditioned on the retained winner and selection history. Treat each history winner as positive evidence and each generated loser as negative evidence; `adaptationSourceWinnerIds` and `adaptationSourceRejectedIds` identify which outcomes already influenced the trajectory. The revision must include `themes`, `inspiration`, `mediaTypes`, `visualStyle`, `colorPalette`, `contentLevel`, and `avoid`, but never mode or source IDs. Keep it concise and describe transferable preferences rather than a person's identity or likeness.
