...
- Use parallel workers only for (a) both sides of a new initial batch claimed by the same batch owner or (b) one `refill-batch` returned by `agent:next`. Start exactly two fresh workers together for a new initial pair and exactly one fresh worker per claimed refill, up to three at once. On recovery, never regenerate a job carrying `terminalStatus`; process only unfinished work. Process ordinary challenger jobs one at a time with one fresh worker each.
...
5. At coordinator startup or restart, run `npm run agent:next -- --resume --wait-ms 0 --max-refills 3`. Process the returned ordinary/initial job or refill batch if printed, then repeat the same recovery command until it prints no JSON. Refill recovery returns up to three unterminated active refills together. Initial jobs include their durable `batchOwnerToken`. Do not use `--resume` in the normal polling loop.
6. Repeatedly run `npm run agent:next -- --wait-ms 30000 --max-refills 3`. Ordinary challenger and initial work has priority. When neither is claimable, the helper atomically claims up to three oldest pending refills and emits one `refill-batch`. Never use a wait longer than 30 seconds or a refill limit above three.
...
- For `kind: "refill-batch"`, require a non-empty `jobs` array of at most three entries and require every entry to have `kind: "refill"`. Spawn exactly one fresh native image-generation subagent per entry, together in parallel. Give each worker only its complete job JSON and its own work directory. Never combine refill prompts or images. Complete or fail every claimed refill independently even if a sibling worker fails, then return to waiting.
