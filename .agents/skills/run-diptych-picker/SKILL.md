...
7. The monitor repeatedly runs `npm run agent:next -- --wait-ms 30000 --max-refills <workerLimit>`. Ordinary challenger, initial, and interactive source-profile work has priority; prompt-card editor/blender work and cached leaderboard-profile analysis follow those interactive jobs and still precede refill claims. When none is claimable, the helper atomically claims up to `workerLimit` oldest pending refills and emits one `refill-batch`. Never use a wait longer than 30 seconds, a refill limit above 3, or a limit above immediately available worker slots.
