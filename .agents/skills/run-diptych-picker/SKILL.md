...
description: Run the Diptych Picker web app with the root interactive session launched through `multi-cli codex/personal` supervising a dedicated persistent mailbox-monitor subagent. Use when starting, operating, resuming, or monitoring the local preference game and processing generation jobs from its repo-local mailbox.
---
...
- **Image workers (fresh subagents):** Generate exactly one candidate each through native image generation, write only their assigned `image.png` and `proposal.json`, report paths to the monitor, and exit. Never reuse a worker for another candidate. Spawn every image worker with the explicit `gpt-5.6-terra` model override at `high` reasoning effort and without inherited turns; never let image workers inherit the profile's default subagent model. A single validation-driven retry may escalate to `gpt-5.6-sol`.
...
- **Leaderboard-profile workers (fresh subagents):** Inspect one immutable, content-addressed set of two through four established current pool leaders, including imported seeds when they lead, write only their assigned `profile.json`, report its path to the monitor, and exit. They synthesize shared visual qualities without modifying or regenerating any source.
...
- Never launch a nested CLI or model process, including another `multi-cli` session, or use an external image API. The root session, monitor subagent, and image-worker subagents are the complete agent tree.
...
- The monitor must use a fresh subagent for every generated candidate. Spawn candidate-generation workers with `model: "gpt-5.6-terra"`, `reasoning_effort: "high"`, and `fork_turns: "none"`. Each worker must read and follow the `imagegen` skill, use native image generation, inspect its exact saved PNG at original detail, and return one PNG path plus the structured proposal. If that handoff fails visual validation, the one permitted regeneration may use `gpt-5.6-sol` with the same reasoning effort and empty fork. Other analysis-worker kinds may use the configured default model.
- The monitor may use parallel workers only for (a) both sides of a new initial batch claimed by the same batch owner, (b) one `initial-import-fill-batch`, (c) one `refill-batch`, or (d) one `import-annotation-batch` returned by `agent:next`. Start exactly two fresh workers together for a new initial pair and exactly one fresh worker per claimed initial-import-fill, refill, or annotation entry, never exceeding `workerLimit`. On recovery, never regenerate a job carrying `terminalStatus`; process only unfinished work. Process ordinary challenger jobs one at a time with one fresh worker.
...
3. Poll the configured local app URL for at most 30 seconds, then verify `/api/game` reports `X-Diptych-Generation-Provider: agent` before declaring readiness. If startup fails or the header is not `agent`, stop the process started by this session, report the error, and remain available; do not start a nested CLI or model process.
4. Spawn exactly one long-lived monitor subagent. Give it the repository path, `workerLimit`, the app URL, whether the root started the app, `generationWorkerModel=gpt-5.6-terra`, `generationWorkerRetryModel=gpt-5.6-sol`, `generationWorkerReasoning=high`, and instructions to read this `SKILL.md` plus [references/job-protocol.md](references/job-protocol.md) completely before touching the mailbox. `workerLimit` is the shared one-through-three fresh-worker-slot bound across image-generation and analysis work, not an image-only limit. Do not ask the monitor to return after one poll or one job.
5. The monitor records readiness with `npm run agent:heartbeat -- --status waiting`.
6. At monitor startup or restart, it runs `npm run agent:next -- --resume --wait-ms 0 --max-refills <workerLimit>`. It processes returned unfinished challenger/initial work, `import-annotation-batch` work, `initial-import-fill-batch` work, cached analysis, or refill batches, then repeats the same recovery command until it prints no JSON. Initial jobs include their durable `batchOwnerToken`. Do not use `--resume` in the normal polling loop.
...
- For `kind: "prompt-card-writer"`, process the one claimed job with one fresh prompt-card writer worker. Require zero through five normalized `sources` plus optional `guidance`, with at least one image or non-empty guidance. Generated-favorite sources have unique `candidateId` values but may share identical image bytes; uploaded seed sources are content-distinct and omit `candidateId`. Resolve every `sourceImage.path` beneath the data root, give the worker all and only those local images plus their display-safe concept/style metadata and exact guidance, and do not call image generation. Preserve the exact candidate IDs, unique image SHA-256 digests, and optional text digest through the completion helper.
...
- For `kind: "initial-import-fill-batch"`, require a non-empty `jobs` array of at most `workerLimit` entries and require every entry to have `kind: "initial-import-fill"`, one import session ID, one attempt ID, and a nonempty preference seed. Spawn exactly one fresh native image-generation worker per entry, together in parallel, including all three worker slots for a three-entry batch. Give each worker only its complete job JSON and its own work directory. It generates one independent standalone square candidate from the clean session preference seed, has no retained winner or comparison evidence, and must omit `preferenceRevision`. Complete or fail every entry independently.
...
- For an initial-import-fill job, use only its authoritative `preferenceSeed`. It has no retained winner, rejected candidate, comparison history, adaptive revision, or winner-pinned ownership. Produce one novel standalone starter candidate and omit `preferenceRevision`.
...
- Inspect the exact saved `image.png` at original detail before returning. Reject and regenerate a blank, near-blank, placeholder, geometric stand-in, UI mockup, or any visible result that does not depict the requested subject and medium, even when its dimensions and file format are valid.
...
2. Delegate to a fresh worker, then independently inspect both its proposal and the exact saved PNG at original detail. Do not treat file existence, PNG metadata, or a plausible proposal as visual validation. Reject blank or near-blank output, placeholders, geometric stand-ins, UI mockups, and any visible image that contradicts the requested subject, medium, count, composition, or another explicit preference-seed constraint; ask that worker to revise or regenerate, and fail the job as `invalid-output` rather than publish an out-of-brief result if it cannot comply.
