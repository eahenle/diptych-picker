...
- Shape future challengers through **Preferences**, with separate guidance for themes, inspiration, media, visual style, palette, content range, and things to avoid. The title-row **Static / Adaptive** control applies to the entire profile: Static prevents model edits, while Adaptive uses winning generated images as positive evidence and rejected generated images as negative evidence, with separate bounded provenance lists for both outcomes. Future workers receive that evidence and recent comparison history when authoring complete profile revisions. Saving or adopting changed profile fields clears candidates buffered under the earlier brief, replaces their capacity with jobs carrying the new seed, and discards any earlier-brief result after its already-running worker exits; provenance-only updates leave buffered work stable. Image workers must treat the current composed preference seed as authoritative. The modal stays openable while a selection waits; Save queues the edited profile with animated feedback and applies it automatically when the challenger arrives.
